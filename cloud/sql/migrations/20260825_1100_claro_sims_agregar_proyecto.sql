-- claro_sims: agregar `proyecto` (INT(11) NULL) inmediatamente despues de
-- `id`. Espejo de 20260825_1000_movistar_sims_agregar_proyecto.sql.
--
-- Motivo:
--   Cada SIM M2M de Claro se usa dentro de un proyecto interno del grupo
...
--   datacount_*, datarocket_*, movistar_sims), todas con `proyecto int(11)
--   NULL` suelto. El ABM solo ofrece proyectos de `tipo = 'I'` (internos) en
--   el desplegable.
--
--   La columna NO la toca la sincronizacion con Autogestion Empresas (el
--   upsert de api/claro_sims_sync.php enumera sus columnas explicitamente),
--   asi que la asignacion manual sobrevive a cada corrida — igual que
...
SET @sql := IF(@exists = 0,
  'ALTER TABLE claro_sims ADD COLUMN `proyecto` INT(11) NULL DEFAULT NULL AFTER `id`',
  'DO 0'
);
...
SET @exists := (
  ...
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME   = 'claro_sims'
    AND INDEX_NAME   = 'idx_claro_sims_proyecto'
);
SET @sql := IF(@exists = 0,
  'ALTER TABLE claro_sims ADD INDEX `idx_claro_sims_proyecto` (`proyecto`)',
  'DO 0'
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
