-- Tabla `clarosims`: catalogo de SIMs M2M administradas via el portal de
-- Autogestion Empresas de Claro. Espejo estructural de `movistarsims`:
-- cada fila representa una linea con su ICCID (icc) como identificador
-- natural unico, y refleja el estado devuelto por el portal (estado
-- general, GPRS, LTE, limite de datos, IMEI, MSISDN). El campo `nombre`
-- corresponde al alias editable de la linea. `actualizado` guarda el
-- timestamp del ultimo sync desde el portal.
...
CREATE TABLE IF NOT EXISTS `clarosims` (
