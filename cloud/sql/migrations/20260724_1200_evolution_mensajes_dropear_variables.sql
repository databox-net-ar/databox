-- Dropea la columna `variables` de `evolution_mensajes`. El ABM cloud ya no la
-- expone ni la guarda: el cuerpo del mensaje viaja completo en `cuerpo` y las
-- variables (si las hubiera) van en `parametros`.
--
...
SET @sql := (
  SELECT IF(
    ...
    'ALTER TABLE `evolution_mensajes` DROP COLUMN `variables`',
    'SELECT 1'
  )
);
