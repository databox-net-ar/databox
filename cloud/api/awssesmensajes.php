<?php
// api/awssesmensajes.php
// ABM de mensajes AWS SES. Lee/escribe sobre la tabla `awssesmensajes`
// definida en db/schema.sql.
//   GET    api/awssesmensajes.php          -> listado con filtros (query string)
//   GET    api/awssesmensajes.php?id=N     -> registro individual
//   POST   api/awssesmensajes.php          -> alta (JSON body)
//   PUT    api/awssesmensajes.php?id=N     -> modificacion (JSON body)
//   DELETE api/awssesmensajes.php?id=N     -> baja
// Respuesta siempre {ok: true, data: ...} u {ok: false, error: '...'} (STACK.md sec. 10).

require_once __DIR__ . '/db.php';
require_once __DIR__ . '/lib/auth_check.php';

const SES_MSG_COLS = "id, fecha, proyecto, canal, plantilla, remitente, remite,
...
// mas el `nombre` del canal traido por LEFT JOIN a `awssescanales`.
const SES_MSG_COLS_LIST = "m.id, m.fecha, m.proyecto, m.canal, m.plantilla,
...
try {
    requirePermCrud('plataformas.awsses.mensajes');
    $pdo    = db();
...
function handleList(PDO $pdo, array $q): void {
    ...
    $sql = "
        SELECT " . SES_MSG_COLS_LIST . "
        FROM awssesmensajes m
        LEFT JOIN awssescanales c ON c.id = m.canal
        {$sqlWhere}
...
function handleGetOne(PDO $pdo, int $id): void {
    $stmt = $pdo->prepare("SELECT " . SES_MSG_COLS . " FROM awssesmensajes WHERE id = :id");
    $stmt->execute([':id' => $id]);
    $row = $stmt->fetch();
...
function handleUpdate(PDO $pdo, int $id, array $in): void {
    $exists = $pdo->prepare('SELECT id FROM awssesmensajes WHERE id = :id');
...
function handleDelete(PDO $pdo, int $id): void {
    $stmt = $pdo->prepare('DELETE FROM awssesmensajes WHERE id = :id');
    $stmt->execute([':id' => $id]);
