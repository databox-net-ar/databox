...
/**
 * Firma y ejecuta un POST/PUT REST contra un endpoint AWS estilo "REST-JSON"
 * (SES v2, SNS v2, EventBridge, etc). A diferencia de aws_json_rpc(), aca:
 *
 *   - la URI puede ser cualquier path (ej: '/v2/email/outbound-emails');
 *   - el Content-Type es 'application/json' (no 'application/x-amz-json-1.x');
 *   - no se envia el header X-Amz-Target.
 *
 * @param string $accessKey Access Key ID del IAM user o rol.
 * @param string $secretKey Secret Access Key.
 * @param string $region    Ej: 'us-east-1'.
 * @param string $service   Signing name (ej: 'ses', 'sns').
 * @param string $host      Ej: 'email.us-east-1.amazonaws.com'.
 * @param string $method    'POST' | 'PUT' | 'GET' | 'DELETE'.
 * @param string $path      URI canonica, empieza con '/' (ej: '/v2/email/outbound-emails').
 * @param array  $payload   Body como array; se serializa a JSON (vacio para GET/DELETE).
 *
 * @return array { status:int, body:string, decoded:array|null }
 */
function aws_rest_json(
    string $accessKey,
    string $secretKey,
    string $region,
    string $service,
    ...
    $body         = $hasBody ? json_encode($payload, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) : '';
    $ts           = time();
    $now          = gmdate('Ymd\THis\Z', $ts);
    $dateStmp     = gmdate('Ymd', $ts);
    $payloadHash  = hash('sha256', $body);
