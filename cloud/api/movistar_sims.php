...
// el nombre del proyecto asignado (`movistar_sims.proyecto` guarda solo el id).
// Sin el prefijo, `nombre` y `id` quedarian ambiguos entre las dos tablas.
const MSIM_COLS = "s.id, s.proyecto, p.nombre AS proyecto_nombre, s.nombre, s.alias, s.linea, s.icc, s.estado, s.estado_gprs, s.estado_lte, s.limite_datos, s.consumo_datos, s.imei, s.msisdn, s.en_uso, s.actualizado, s.ultimo_trafico, s.tags";
const MSIM_FROM = "FROM movistar_sims s LEFT JOIN proyectos p ON p.id = s.proyecto";
...
function handleGetOne(PDO $pdo, int $id): void {
    $stmt = $pdo->prepare("SELECT " . MSIM_COLS . " " . MSIM_FROM . " WHERE s.id = :id");
    $stmt->execute([':id' => $id]);
    $row = $stmt->fetch();
