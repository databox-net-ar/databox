<?php
// api/awssescanales.php
// ABM de canales AWS SES. Lee/escribe sobre la tabla `awssescanales`
// definida en db/schema.sql.
//   GET    api/awssescanales.php          -> listado con filtros (query string)
//   GET    api/awssescanales.php?id=N     -> registro individual
//   POST   api/awssescanales.php          -> alta (JSON body)
//   PUT    api/awssescanales.php?id=N     -> modificacion (JSON body)
//   DELETE api/awssescanales.php?id=N     -> baja
// Respuesta siempre {ok: true, data: ...} u {ok: false, error: '...'} (STACK.md sec. 10).

require_once __DIR__ . '/db.php';
require_once __DIR__ . '/lib/auth_check.php';

const SES_CH_COLS = "id, uuid, nombre, correo, servidor, usuario, contrasena, habilitado";

header('Content-Type: application/json; charset=utf-8');

try {
    requirePermCrud('plataformas.awsses.canales');
    $pdo    = db();
...
function handleGetOne(PDO $pdo, int $id): void {
    $stmt = $pdo->prepare("SELECT " . SES_CH_COLS . " FROM awssescanales WHERE id = :id");
    $stmt->execute([':id' => $id]);
    $row = $stmt->fetch();
...
function handleUpdate(PDO $pdo, int $id, array $in): void {
    $exists = $pdo->prepare('SELECT id FROM awssescanales WHERE id = :id');
...
function handleDelete(PDO $pdo, int $id): void {
    $stmt = $pdo->prepare('DELETE FROM awssescanales WHERE id = :id');
    $stmt->execute([':id' => $id]);
